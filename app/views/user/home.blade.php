...
<div class="row">
	<div class="col-md-6 col-md-offset-3">
		<div class="authed">
			...
			<a href="{{ URL::to('games') }}" class="btn btn-lg btn-success">Join Game</a> <a href="{{ URL::to('sponsors') }}" class="btn btn-lg btn-primary">Sponsor a Friend</a>
			@else
			...
			<p>You have <strong>{{ $auth->gameusers->count() }}</strong> registered game users.</p>
			<p>You own <strong>{{ $auth->ownedgroups->count() }}</strong> groups and <strong>{{ $auth->ownedgames->count() }}</strong> games.</p>
			<a href="{{ URL::to('games') }}" class="btn btn-lg btn-success">Join Game</a>
			@endif
		</div>
	</div>
</div>
